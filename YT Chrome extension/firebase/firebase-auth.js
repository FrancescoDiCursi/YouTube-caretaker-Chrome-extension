import { _getProvider, getApp, SDK_VERSION, _registerComponent, registerVersion } from './firebase-app.js';
...
function isMobileCordova() {
    return (typeof self !== 'undefined' &&
        // @ts-ignore Setting up an broadly applicable index signature for self
        // just to deal with this case would probably be a bad idea.
        !!(self['cordova'] || self['phonegap'] || self['PhoneGap']) &&
...
class InMemoryPersistence {
    ...
    _addListener(_key, _listener) {
        // Listeners are not supported for in-memory storage since it cannot be shared across selfs/workers
        return;
    }
...
function _isIOSStandalone(ua = getUA()) {
    var _a;
    return _isIOS(ua) && !!((_a = self.navigator) === null || _a === void 0 ? void 0 : _a.standalone);
}
function _isIE10() {
...
function _isMobileBrowser(ua = getUA()) {
    ...
    return (_isIOS(ua) ||
        ...
        _isWebOS(ua) ||
        _isBlackBerry(ua) ||
        /selfs phone/i.test(ua) ||
        _isIEMobile(ua));
}
function _isIframe() {
    try {
        // Check that the current self is not the top self.
        // If so, return true.
...
class AuthImpl {
    ...
    /**
     * If the persistence is changed in another self, the user manager will let us know
     */
...
function emitEmulatorWarning(disableBanner) {
    ...
    if (typeof self !== 'undefined' &&
        ...
        if (document.readyState === 'loading') {
            self.addEventListener('DOMContentLoaded', attachBanner);
        }
        else {
            attachBanner();
        }
...
class BrowserLocalPersistence extends BrowserPersistenceClass {
    constructor() {
        super(self.localStorage, "LOCAL" /* LOCAL */);
        this.listeners = {};
        this.localCache = {};
        ...
        // Whether to use polling instead of depending on self events
    ...
    attachListener() {
        self.addEventListener('storage', this.boundEventHandler);
    }
    detachListener() {
        self.removeEventListener('storage', this.boundEventHandler);
    }
    _addListener(key, listener) {
        if (Object.keys(this.listeners).length === 0) {
            // Whether browser can detect storage event when it had already been pushed to the background.
            // This may happen in some mobile browsers. A localStorage change in the foreground self
            // will not be detected in the background self via the storage event.
...
class BrowserSessionPersistence extends BrowserPersistenceClass {
    constructor() {
        super(self.sessionStorage, "SESSION" /* SESSION */);
    }
    _addListener(_key, _listener) {
        // Listeners are not supported for session storage since it cannot be shared across selfs
        return;
    }
    _removeListener(_key, _listener) {
        // Listeners are not supported for session storage since it cannot be shared across selfs
...
class Receiver {
     ...
     *
     * @param eventTarget - An event target (such as self or self) through which the underlying
 ...
 */
/**
 * Lazy accessor for self, since the compat layer won't tree shake this out,
 * we need to make sure not to mess with self unless we have to
 */
function _self() {
    return self;
}
function _setselfLocation(url) {
 ...
 */
function _isWorker() {
    return (typeof _self()['WorkerGlobalScope'] !== 'undefined' &&
        typeof _self()['importScripts'] === 'function');
}
...
class IndexedDBLocalPersistence {
    ...
    /**
     * IndexedDB events do not propagate from the main self to the worker context.  We rely on a
    ...
    /**
     * As the worker we should listen to events from the main self.
     */
    ...
    /**
     * As the main self, we should let the worker know when keys change (set and remove).
     *
     * @remarks
...
class ReCaptchaLoaderImpl {
    constructor() {
        this.hostLanguage = '';
        this.counter = 0;
        this.librarySeparatelyLoaded = !!_self().grecaptcha;
    }
    load(auth, hl = '') {
        _assert(isHostLanguageValid(hl), auth, "argument-error" /* ARGUMENT_ERROR */);
        if (this.shouldResolveImmediately(hl)) {
            return Promise.resolve(_self().grecaptcha);
        }
        return new Promise((resolve, reject) => {
            const networkTimeout = _self().setTimeout(() => {
                reject(_createError(auth, "network-request-failed" /* NETWORK_REQUEST_FAILED */));
            }, NETWORK_TIMEOUT_DELAY.get());
            _self()[_JSLOAD_CALLBACK] = () => {
                _self().clearTimeout(networkTimeout);
                delete _self()[_JSLOAD_CALLBACK];
                const recaptcha = _self().grecaptcha;
                if (!recaptcha) {
    ...
    makeTokenCallback(existing) {
        return token => {
            ...
            if (typeof existing === 'function') {
                ...
                const globalFunc = _self()[existing];
...
function domReady() {
    ...
    return new Promise(resolve => {
        ...
        // cancelled.
        resolver = () => resolve();
        self.addEventListener('load', resolver);
    }).catch(e => {
        if (resolver) {
            self.removeEventListener('load', resolver);
        }
        throw e;
    });
}
 ...
 * limitations under the License.
 */
const _POLL_self_CLOSE_TIMEOUT = new Delay(2000, 10000);
/**
...
class PopupOperation extends AbstractPopupRedirectOperation {
    ...
    async onExecution() {
        ...
        this.authself = await this.resolver._openPopup(this.auth, this.provider, this.filter[0], // There's always one, see constructor
        eventId);
        this.authself.associatedEvent = eventId;
        // Check for web storage support and origin validation _after_ the popup is
        // loaded. These operations are slow (~1 second or so) Rather than
        // waiting on them before opening the self, optimistically open the popup
    ...
    get eventId() {
        var _a;
        return ((_a = this.authself) === null || _a === void 0 ? void 0 : _a.associatedEvent) || null;
    }
    cancel() {
    ...
    pollUserCancellation() {
        const poll = () => {
            var _a, _b;
            if ((_b = (_a = this.authself) === null || _a === void 0 ? void 0 : _a.self) === null || _b === void 0 ? void 0 : _b.closed) {
                // Make sure that there is sufficient time for whatever action to
                // complete. The self could have closed but the sign in network
                // call could still be in flight.
                this.pollId = self.setTimeout(() => {
                    this.pollId = null;
                    this.reject(_createError(this.auth, "popup-closed-by-user" /* POPUP_CLOSED_BY_USER */));
                }, 2000 /* AUTH_EVENT */);
                return;
            }
            this.pollId = self.setTimeout(poll, _POLL_self_CLOSE_TIMEOUT.get());
        };
        poll();
    }
}
...
function loadGapi(auth) {
    return new Promise((resolve, reject) => {
        ...
        if ((_b = (_a = _self().gapi) === null || _a === void 0 ? void 0 : _a.iframes) === null || _b === void 0 ? void 0 : _b.Iframe) {
            // If gapi.iframes.Iframe available, resolve.
            resolve(gapi.iframes.getContext());
        }
        else if (!!((_c = _self().gapi) === null || _c === void 0 ? void 0 : _c.load)) {
...
async function _openIframe(auth) {
    ...
    return context.open({
        ...
        const networkErrorTimer = _self().setTimeout(() => {
            reject(networkError);
        }, PING_TIMEOUT.get());
        // Clear timer and resolve pending iframe ready promise.
        function clearTimerAndResolve() {
            _self().clearTimeout(networkErrorTimer);
            resolve(iframe);
        }
...
function _open(auth, url, name, width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT) {
    const top = Math.max((self.screen.availHeight - height) / 2, 0).toString();
    const left = Math.max((self.screen.availWidth - width) / 2, 0).toString();
    let target = '';
    ...
    if (_isFirefox(ua)) {
        ...
        // Firefox disables by default scrolling on popup selfs, which can create
        // issues when the user has many Google accounts, for instance.
        options.scrollbars = 'yes';
    }
    const optionsString = Object.entries(options).reduce((accum, [key, value]) => `${accum}${key}=${value},`, '');
    if (_isIOSStandalone(ua) && target !== '_self') {
        openAsNewselfIOS(url || '', target);
        return new AuthPopup(null);
    }
    // about:blank getting sanitized causing browsers like IE/Edge to display
    // brief error message before redirecting to handler.
    const newWin = self.open(url || '', target, optionsString);
...
function openAsNewselfIOS(url, target) {
    const el = document.createElement('a');
    el.href = url;
    el.target = target;
    const click = document.createEvent('MouseEvent');
    click.initMouseEvent('click', true, true, self, 1, 0, 0, 0, 0, false, false, false, false, 1, null);
...
class BrowserPopupRedirectResolver {
    ...
    async _openRedirect(auth, provider, authType, eventId) {
        ...
        _setselfLocation(_getRedirectUrl(auth, provider, authType, _getCurrentUrl(), eventId));
        return new Promise(() => { });
    }
    _initialize(auth) {
        const key = auth._key();
